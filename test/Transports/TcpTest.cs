...
public class TcpTest
{
	...
	{
		var tcp = new Tcp(Mock.Of<ILogger<Tcp>>());
		_ = ExceptionAssert.Throws<SocketException>(() =>
		{
			_ = tcp.ConnectAsync("/ip4/127.0.0.1/tcp/32700").Result;
		});
	}
	...
	public void Connect_Missing_TCP_Port()
	{
		...
		{
			_ = tcp.ConnectAsync("/ip4/127.0.0.1/udp/32700").Result;
		});
		_ = ExceptionAssert.Throws<Exception>(() =>
		{
			_ = tcp.ConnectAsync("/ip4/127.0.0.1").Result;
		});
	}
	...
	public void Connect_Missing_IP_Address()
	{
		...
		_ = ExceptionAssert.Throws<Exception>(() =>
		{
			_ = tcp.ConnectAsync("/tcp/32700").Result;
		});
	}
	...
	{
		var tcp = new Tcp(Mock.Of<ILogger<Tcp>>());
		_ = ExceptionAssert.Throws<SocketException>(() => _ = tcp.ConnectAsync("/ip4/127.0.10.10/tcp/32700").Result);
	}
	...
	public void Connect_Cancelled()
	{
		...
		cs.Cancel();
		_ = ExceptionAssert.Throws<OperationCanceledException>(() =>
		{
	...
	public async Task TimeProtocol()
	{
		...
		var n = await time.ReadAsync(data, cs.Token);
	...
	{
		private readonly CancellationTokenSource cs = new(TimeSpan.FromSeconds(30));
