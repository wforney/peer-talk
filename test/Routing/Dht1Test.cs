...
public class Dht1Test
{
	...
	public async Task StoppedEventRaised()
	{
		...
		var dht = new Dht1(loggerFactory, notificationService) { Swarm = swarm };
		bool stopped = false;
		var sub = notificationService.Subscribe<Dht1.Stopped>(m => stopped = true);
	...
	public async Task ProcessFindNodeMessage_Closest()
	{
		...
		var dht = new Dht1(loggerFactory, notificationService) { Swarm = swarm, CloserPeerCount = 3 };
		await dht.StartAsync();
		try
		{
			dht.RoutingTable.Add(other);
