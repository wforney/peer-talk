namespace PeerTalk.Routing
{
	...
	{
		private readonly KBucket<RoutingPeer> Peers = new KBucket<RoutingPeer>();
		...
		/// </summary>
		public bool Contains(Peer peer) => Peers.Contains(new RoutingPeer(peer));

		/// <summary>
		///   Find the closest peers to the peer ID.
		/// </summary>
		public IEnumerable<Peer> NearestPeers(MultiHash id) => Peers.Closest(Key(id)).Select(r => r.Peer);
		...
		static public byte[] Key(MultiHash id) => MultiHash.ComputeHash(id.ToArray(), "sha2-256").Digest;
	}
}
