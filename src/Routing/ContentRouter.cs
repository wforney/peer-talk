namespace PeerTalk.Routing
{
	...
	public class ContentRouter : IDisposable
	{
		...
		private readonly ConcurrentDictionary<string, List<ProviderInfo>> content = new ConcurrentDictionary<string, List<ProviderInfo>>();
		...
		public IEnumerable<MultiHash> Get(Cid cid) =>
			content.TryGetValue(Key(cid), out List<ProviderInfo> providers)
				? providers
					.Where(p => DateTime.Now < p.Expiry)
					.Select(p => p.PeerId)
				: Enumerable.Empty<MultiHash>();
